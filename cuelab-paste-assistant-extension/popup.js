...
const ACCEPTED_BRIDGE_SCHEMAS = new Set([
  'aural-grid-beatpulse-bridge-v1',
  'cuelab-beatpulse-bridge-v1'
]);
...
function normalizePayload(raw) {
  ...
  if (ACCEPTED_BRIDGE_SCHEMAS.has(raw.schema)) {
    if (!raw.qa_review) raw.qa_review = { checked: false, warnings: ['Bridge predates Golden Check. Review the transfer manually.'] };
    return raw;
  }
  ...
  return {
    ...
    qa_review: raw.qa_review || { checked: false, warnings: ['No Golden Check metadata supplied. Review the transfer manually.'] }
  };
}
...
function payloadSummary(payload) {
  ...
  const warnings = payload.qa_review && Array.isArray(payload.qa_review.warnings) ? payload.qa_review.warnings.length : 0;
  return `${title}${artist ? ` - ${artist}` : ''}. ${sections} section${sections === 1 ? '' : 's'} loaded.${warnings ? ` ${warnings} Golden Check note${warnings === 1 ? '' : 's'} to review.` : ' Golden Check clear.'}`;
}
