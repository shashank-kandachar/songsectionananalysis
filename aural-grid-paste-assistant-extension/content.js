(() => {
  ...
  function preflightReport() {
    ...
    return {
      ...
      fieldsFound: found,
      missing,
      ready: !!state.queue.length && rowsFound >= expectedRows && !missing.length
    };
  }
  ...
  async function fillAllVisibleFields() {
    ...
    const stats = { filled: 0, skipped: 0, missing: 0 };
    ...
    state.lastReport = {
      type: 'fill',
      message: `${stats.filled} filled, ${stats.skipped} already matched, ${stats.missing} missing.`,
      detail: 'Review the dashboard before submitting.'
    };
    render();
